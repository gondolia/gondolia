...
export function ProductCard({ product }: ProductCardProps) {
  ...
  const getStockStatus = () => {
    ...
    // For bundles and parametric products: always show available (stock check happens on components)
    if (product.productType === 'bundle' || product.productType === 'parametric') {
      return 'available';
    }
    // For simple products: assume available if stockQuantity is undefined/null
    // (detailed stock check happens at add-to-cart time)
  ...
  // Only simple products can be added to cart directly from the card
  // Bundles, parametric, and variants require configuration on detail page
  const canAddToCart = product.productType === 'simple';

  return (
    <div className="h-full flex flex-col">
      <Link href={`/products/${product.id}`} className="flex-1">
        <Panel className="h-full hover:shadow-lg transition-shadow cursor-pointer flex flex-col">
